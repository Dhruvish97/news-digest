...
# ── Load .env if present (used by launchd scheduler) ──────────────────────────
_env_file = Path(__file__).parent / ".env"
try:
    _env_lines = _env_file.read_text().splitlines()
...
def fetch_news() -> list[dict]:
    """Call Claude with built-in web_search (server-side) to get today's top 10."""
    client = anthropic.Anthropic()
    today  = datetime.now().strftime("%B %d, %Y")

    prompt = f"""Today is {today}. Search the web for the TOP 10 most globally important news stories from the LAST 24 HOURS.
...
- "category": one of Politics, Tech, Finance, Science, Environment, Health, War & Conflict, Business, Society, Other
- "source": news outlet name
- "importance_score": 1-10
- "region": e.g. Global, United States, Europe, Asia, Middle East
"""

    print("🔍 Fetching today's top news via Claude web search...")
    ...
    # Prefer the block that looks like JSON; otherwise fall back to the last text block
    final_text = next(
        (t for t in text_blocks if t.strip().startswith("[") or t.strip().startswith("```")),
        text_blocks[-1] if text_blocks else None,
    )
    if not final_text:
...
<footer>
  ...
  Generated by <a href="https://claude.ai">Claude</a> ({MODEL})
...
def main():
    if not os.environ.get("ANTHROPIC_API_KEY"):
        ...
        print()
        print("Either:")
        print("  1. Edit ~/Claude/news_digest/.env and replace sk-ant-YOUR_KEY_HERE")
        print("  2. Or add to ~/.zshrc:  export ANTHROPIC_API_KEY=\"sk-ant-...\"")
        raise SystemExit(1)
